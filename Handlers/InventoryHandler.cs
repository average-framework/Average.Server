...
namespace Average.Server.Handlers
{
    internal class InventoryHandler : IHandler
    {
        ...
        public InventoryHandler(ClientService clientService, InventoryService inventoryService, RpcService rpcService, UserService userService)
        {
        ...
        [ServerEvent("inventory:giveitem")]
        private void OnGiveItem(Client client, int playerId, string itemName, int itemCount)
        {
            var target = _clientService.Clients.Find(x => int.Parse(x.Player.Handle) == playerId);
            _inventoryService.GiveItem(target, itemName, itemCount);
        }
    }
}
